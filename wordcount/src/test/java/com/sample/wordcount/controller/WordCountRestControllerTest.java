...
public class WordCountRestControllerTest {
	...
	@InjectMocks
	WordCountRestController wController;

	@Test
	@DisplayName("getWordsWithMinSize 1 : test with input size 3 ")
	public void getWordsWithMinSize_1() throws Exception {
		mockMvc.perform(get("/wordcount/words?minlen=3").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ \"mama momo\", \"m@123\" ] }")

		).andDo(print()).andExpect(jsonPath("$.minLength").value("3"))
				.andExpect(jsonPath("$.wordsWithSize", hasSize(3))).andExpect(status().is2xxSuccessful());
	}

	@Test
	@DisplayName("getWordsWithMinSize 2 : test with default size 5 ")
	public void getWordsWithMinSize_2() throws Exception {
		mockMvc.perform(get("/wordcount/words").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ \"mama momo\", \"m@123\" ] }")

		)
				// .andDo(print())
				.andExpect(jsonPath("$.minLength").value("5")).andExpect(jsonPath("$.wordsWithSize", hasSize(1)))
	...
	public void getWordCountWithChar_1() throws Exception {
		mockMvc.perform(get("/wordcount/numberofwords?startingchar=A").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ \"mama momo\", \"m@123\" ] }")

		)
				// .andDo(print())
				.andExpect(jsonPath("$.startingChar").value("A"))
	...
	public void getWordCountWithChar_2() throws Exception {
		mockMvc.perform(get("/wordcount/numberofwords").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ \"mama momo\", \"m@123\" ] }")

		)
				// .andDo(print())
				.andExpect(jsonPath("$.startingChar").value("M"))
	...
	public void getSummary_1() throws Exception {
		mockMvc.perform(get("/wordcount/summary").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ \"mama momo\", \"m@123\" ] }")
	...
	public void getSummary_2() throws Exception {
		mockMvc.perform(get("/wordcount/summary?startingchar=X&minlen=2").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ \"mama momo\", \"m@123\", \"Xyz\" ] }")

		)
				// .andDo(print())
				.andExpect(jsonPath("$.startingChar").value("X"))
				.andExpect(jsonPath("$.wordCountStartingWithInputChar").value("1"))
				.andExpect(jsonPath("$.minLength").value("2")).andExpect(jsonPath("$.wordsWithSize", hasSize(4)))
				.andExpect(status().is2xxSuccessful());
	}

	@Test
	@DisplayName("getSummary 3 : test with empty input list")
	public void getSummary_3() throws Exception {
		mockMvc.perform(get("/wordcount/summary?startingchar=X&minlen=2").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"list\" : [ ] }")

		).andDo(print()).andExpect(jsonPath("$.list").value(ErrorConstants.INPUT_LIST_EMPTY_MSG))
